...
const aggregateByPostType = (data, selectedAccount = ALL_ACCOUNTS) => {
  ...
  const aggregatedData = Object.values(postTypeGroups).map(group => {
    ...
    ['views', 'post_reach', 'engagement_total', 'likes', 'comments', 'shares', 'saves', 'follows'].forEach(metric => {
      let sum = 0;
      let count = 0;
...
const PostTypeView = ({ data, selectedFields }) => {
  ...
  const handleCopyValue = (value, field, rowId) => {
    ...
    // Convert to string and ensure formatting is removed
    // For percentage values, keep decimal places
    let rawValue;
    
    if (field === 'percentage') {
      // For percentage values, keep decimal places but remove % sign
      rawValue = String(value.toFixed(1));
    } else if (typeof value === 'number' || (typeof value === 'string' && !isNaN(value.replace(/\s+/g, '')))) {
      // For other numeric values, remove all non-digits
      rawValue = String(value).replace(/\s+/g, '').replace(/\D/g, '');
    } else {
      // For non-numeric values, keep text as is
    ...
    navigator.clipboard.writeText(rawValue)
      .then(() => {
        ...
        console.log(`Copied ${rawValue} to clipboard`);
      })
      .catch(err => {
  ...
  const CopyButton = ({ value, field, rowId }) => {
    ...
    // Don't show copy button for empty values or null/undefined
  ...
  const handleExportToExcel = async () => {
    try {
      ...
      const result = await window.electronAPI.exportToExcel(
        exportData,
        'instagram-statistik-inlaggstyper.xlsx'
      );
      if (result.success) {
  ...
  const handleExportToCSV = async () => {
    try {
      ...
      const result = await window.electronAPI.exportToCSV(
        exportData,
        'instagram-statistik-inlaggstyper.csv'
      );
      if (result.success) {
  ...
  const formatDataForExport = (data) => {
    ...
    return data.map(item => {
      ...
      for (const field of selectedFields) {
        if (['views', 'post_reach', 'engagement_total', 'likes', 'comments', 'shares', 'saves', 'follows'].includes(field)) {
            ...
            <TableHeader>
              <TableRow>
                ...
                {selectedFields.map(field => {
                  // Only show selected metrics that make sense for this view
                  if (!['views', 'post_reach', 'engagement_total', 'likes', 'comments', 'shares', 'saves', 'follows'].includes(field)) {
                    return null;
                  }
